...
def strip_foreign_contact_email(cache, anchor: str) -> dict:
    ...
    `ot.website@massiveds.com` reached a draft for a company at massive-dynamic.ai. An address on the
...
def draft_one(provider: Provider, cs: CompanyState, voice_override: str | None = None,
    ...
    try:
        ...
        cs.machine_blocks = {k: v for k, v in (parts or {}).items() if isinstance(v, str)}
