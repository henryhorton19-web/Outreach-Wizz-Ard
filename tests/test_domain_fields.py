...
def test_company_domain_prefers_the_operator_website():
    cs = CompanyState(slug="s", name="Synaps", website="https://www.synaps.com/",
                      recipient_domain="mail.other.com")
    assert pipeline.company_domain(cs) == "synaps.com"


def test_company_domain_falls_back_to_research():
    cs = CompanyState(slug="s", name="Synaps",
                      cache={"company": {"resolved_domain": "synaps.com"}})
    assert pipeline.company_domain(cs) == "synaps.com"


def test_company_domain_ignores_the_recipient_domain():
    cs = CompanyState(slug="s", name="Synaps", recipient_domain="gmail.com")
...
def test_editing_the_address_does_not_change_company_domain():
    ...
    cs.recipient_domain = "founders.synaps.io"
