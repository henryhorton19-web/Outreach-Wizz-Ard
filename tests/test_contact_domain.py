...
The bug: research for a company returned chandler@avyn.com and lauren@avyn.com
(wrong person, wrong domain) when the correct answer was jamie@avyn.io. Every
...
def _cache(email="", email_confidence="low", contact_verified=False,
          email_source_url="", company_website="https://avyn.io"):
    return {
        "company": {"name": "Avyn", "website": company_website, "role_exists": False,
...
def test_email_domain_must_match_the_resolved_company_domain():
    ...
    bad = _cache(email="chandler@avyn.com")  # company website is avyn.io
...
def test_matching_domain_with_no_source_is_flagged_as_unverified_guess():
    ...
    guess = _cache(email="jamie@avyn.io", email_source_url="")
    gaps = research._contact_gaps(guess)
    assert any("source" in g for g in gaps)


def test_matching_domain_with_a_real_source_passes():
    good = _cache(email="jamie@avyn.io", email_confidence="high",
                  contact_verified=True, email_source_url="https://avyn.io/team")
    assert research._contact_gaps(good) == []


def test_resolve_domain_prefers_an_explicitly_given_website():
    dom, source = research.resolve_company_domain(name="Avyn", given_website="https://avyn.io/about")[:2]
    assert dom == "avyn.io"
    assert source == "given"
...
def test_post_process_discards_wrong_domain_email():
    bad_cache = _cache(email="chandler@avyn.com")
    processed = research._post_process(bad_cache, "Avyn", "https://avyn.io", [], resolved_domain="avyn.io")
    assert processed["contact"]["email"] == "jamie.someone@avyn.io"
...
def test_contacts_alt_wrong_domain_email_is_discarded():
    raw_cache = _cache(email="jamie@avyn.io", email_source_url="https://avyn.io/team")
    raw_cache["contacts_alt"] = [{"name": "Wrong Alt", "email": "alt@wrong.com"}]
    processed = research._post_process(raw_cache, "Avyn", "https://avyn.io", [], resolved_domain="avyn.io")
...
def test_pattern_fallback_email_generated_when_scraped_email_missing():
    ...
    assert processed["contacts_alt"][0]["email"] == "jsomeone@avyn.io"
...
def test_address_ladder_contains_primary_fallback_first_and_alt_second():
    ...
    assert "jamie.someone@avyn.io" in emails
    assert "jsomeone@avyn.io" in emails
    assert emails.index("jamie.someone@avyn.io") < emails.index("jsomeone@avyn.io")
