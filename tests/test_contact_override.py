...
def _blocked():
    cs = CompanyState(slug="mankinds", name="Mankinds", state=State.error,
                      error="not enough to write from: no named contact was found",
                      cache={"company": {"name": "Mankinds", "what_they_do": "stealth AI",
                                         "resolved_domain": "mankinds.ai"},
                             "contact": {"name": "Unknown", "email": "unknown@mankinds.ai",
...
def test_setting_a_contact_clears_the_block(client):
    _blocked()
    r = client.put("/api/companies/mankinds/contact",
                   json={"name": "Adrien Barrau", "email": "adrien@mankinds.ai"})
    assert r.status_code == 200
    cs = store.get_draft("mankinds")
    c = (cs.cache or {})["contact"]
    assert c["name"] == "Adrien Barrau"
    assert c["email"] == "adrien@mankinds.ai"
...
def test_setting_a_contact_works_before_any_draft_exists(client):
    _blocked()
    assert store.get_draft("mankinds").machine_email is None
    assert client.put("/api/companies/mankinds/contact",
                      json={"name": "Adrien Barrau", "email": "adrien@mankinds.ai"}).status_code == 200


def test_a_name_only_override_is_accepted(client):
    _blocked()
    r = client.put("/api/companies/mankinds/contact", json={"name": "Adrien Barrau"})
    assert r.status_code == 200
    assert (store.get_draft("mankinds").cache or {})["contact"]["name"] == "Adrien Barrau"
...
def test_a_bad_address_is_rejected(client):
    _blocked()
    assert client.put("/api/companies/mankinds/contact",
                      json={"name": "Adrien Barrau", "email": "not-an-email"}).status_code == 400


def test_an_address_off_the_company_domain_is_accepted_but_flagged(client):
    _blocked()
    r = client.put("/api/companies/mankinds/contact",
                   json={"name": "Adrien Barrau", "email": "adrien@gmail.com"})
    assert r.status_code == 200
