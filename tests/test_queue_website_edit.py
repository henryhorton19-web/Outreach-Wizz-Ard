...
def _queue_one(website=None):
    store.upsert_queue("elix_ai", "Elix AI", None, None, list_id="default", website=website)
...
def test_set_a_website_on_a_queued_target(client):
    _queue_one()
    r = client.put("/api/queue/elix_ai/website", json={"website": "https://elix.ai"})
    assert r.status_code == 200
    assert _rec()["website"] == "https://elix.ai"


def test_correct_a_wrong_website(client):
    _queue_one("https://elix-inc.com")
    client.put("/api/queue/elix_ai/website", json={"website": "elix.ai"})
    assert _rec()["website"] == "elix.ai"


def test_clear_a_website_with_an_empty_string(client):
    _queue_one("https://elix.ai")
...
def test_edited_website_reaches_the_draft(client):
    _queue_one()
    client.put("/api/queue/elix_ai/website", json={"website": "https://elix.ai"})
    client.post("/api/queue/elix_ai/draft")
    cs = store.get_draft("elix_ai")
    assert cs is not None
    assert cs.website == "https://elix.ai"
